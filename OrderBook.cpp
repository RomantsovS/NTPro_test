...
void OrderBook::AddSellOrder(int value, double price, client_id_type client_id) {
    orders_sell.push(
        {value, price, Order::Type::sell, client_id, std::chrono::system_clock::now()});
}

std::optional<std::reference_wrapper<const Order>> OrderBook::GetBuyOrder() {
    if (!orders_buy.empty()) return orders_buy.top();

    return std::nullopt;
}

std::optional<std::reference_wrapper<const Order>> OrderBook::GetSellOrder() {
...
std::optional<Deal> OrderBook::MakeDeal() {
    ...
    return Deal(min_amount, order_sell.GetPrice(), order_buy.GetClientID(),
                order_sell.GetClientID(), std::chrono::system_clock::now());
}